...
/**
 * \file    test/phpunit/ContratosFunctionalTest.php
 * \ingroup contratos
 ...
 *
 * @package Testcontratos
 */
class ContratosFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modContratos')->click();
		} else {
			// Disable the module
			$this->byHref('modContratos')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/contratos/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('contratos/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/contratos/admin/about.php');
		$this->authenticate();
		return $this->assertContains('contratos/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('contratoswidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modContratos_ContratosTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modContratos_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
